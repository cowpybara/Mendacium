    ...
    NavHost(
        ...
        composable<ConfigurationScreenRoute> {
            ConfigurationScreen(
                onNavigateToLobbyCustom = { config, finalPlayersList ->
                    ...
                    // Si es local, guardamos directamente la lista con TODOS los nombres personalizados
                    if (gameMode == GameMode.LOCAL) {
                        preGamePlayers = finalPlayersList
                    } else {
        ...
        composable<LobbyScreenRoute>(enterTransition = { fadeIn() }, exitTransition = { fadeOut() }) {
            ...
            LobbyScreen(
                ...
                onStartGame = {
                    if (gameMode == GameMode.LOCAL) {
                        // Pasamos la lista `preGamePlayers` que ya contiene los nombres reales
